...
class GRID2DEMAND:
    ...
    def gen_node_prod_attr(self, node_dict: dict = "", poi_dict: dict = "", return_value: bool = False) -> dict[int, Node]:
    ...
    def save_results_to_csv(self, output_dir: str = "",
        ...
        if node:
            if not hasattr(self, "node_dict"):
                print("Error: node_dict does not exist. Please run load_node() first.")
                return
        ...
        if poi:
            if not hasattr(self, "poi_dict"):
                print("Error: poi_dict does not exist. Please run load_poi() first.")
                return
